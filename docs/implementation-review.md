...
| B-F2 | major | eff | R-24 leg 2 missing: no host-side timeout wrapper around host imports; a hanging `Host::call` wedges the call past the 5 s post-return-only bound. | **Fixed in the pivot (T6)**: `kanbei-vm` supervises every host import — one bounded worker per top-level import, nested imports inline (T6 preserved a `SERVICE_DEPTH` cap, since superseded by T20's scope carried between generation actors), `recv_timeout` per import, a per-`Vm` in-flight ceiling that fails closed once *live* in-flight workers reach it (an abandoned worker keeps counting until it exits, so a genuine hang still consumes its slot), `Host::retire` on timeout, and vm-side terminal state on timeout. The count of abandoned-but-still-running workers is exposed on `Vm::abandoned_host_workers` as observability only; it is deliberately *not* a second fail-closed budget, since a guest could wedge workers and deny host imports. A blocked worker cannot be killed, so retirement invalidates *future* effects; an import already in flight is not cancelled (documented on `Host::retire`). |
...
| ID | Sev | Axis | Finding | Disposition |
|---|---|---|---|---|
| C-F1 | major | fid | No activation-time state-schema validation (R-07/C-07); `PackageManifest.state_schema` is a dead field; `module reset-state` absent entirely (arch:236). | **Fixed in the pivot (T7)**: `PackageManifest` gains `state_key` (schema bump 1→2) binding the module to its head; `activate` validates the existing head's schema against the declared `state_schema` *before any side effect* (typed `StateError::SchemaMismatch`, old head untouched); `state_key`/`state_schema` must be set together. `StateStore::reset_head` + `Session::reset_module_state` start a fresh head and commit a canonical `state_reinitialized` fact. CLI wiring of `module reset-state` deferred (session API only). `PACKAGE_SCHEMA` 1→2 is a hard gate: a pre-bump session's schema-1 config reopens in safe mode, with no migration/upcast in MVP. |
| C-F2 | major | eff | Failed-activation rollback leaks host-op-6 registry publications and per-generation contributions (only `drop_generation` cleans them; a rejected transaction leaves composition state). | **Fixed in the pivot (T7)**: the activation-rollback branch now runs the canonical `teardown_generation` (token-first, unpublishes services + contributions) before draining, so a failed activation leaves nothing published. Pinned by `failed_activation_does_not_leak_services_or_contributions`. |
...
| C-F5 | major | fid | R-01: the epoch counter is mixed into the composition digest — identical compositions digest differently; no-op publishes change EpochId; manifest dedup defeated; the crate's own doc claim is false. | **Fixed in the pivot (T7)**: `composition_canonical_bytes`/`composition_digest` no longer embed `epoch`; the epoch counter still advances for OCC staleness, but EpochId is a pure function of the contribution set, so identical compositions dedup. Crate doc corrected; `epoch_is_not_mixed_into_the_composition_digest` pins it. |
...
| C-F10 | minor | craft | Disposal logic triplicated; public `Generation::dispose` is dead + weaker (leaks contributions). | **Fixed in the pivot (T7)**: disposal now routes through one canonical `ModuleHost::teardown_generation` (broker grants → token → services → contributions), shared by `Host::retire`, `Generation::dispose`, `drop_generation`, and the activation rollback; all paths now agree on token-before-effects. The non-blocking vm `retire` vs blocking drain distinction remains, by design. |
